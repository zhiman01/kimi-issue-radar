...
1. **数据范围**：432 条是近期活跃 issue 的采样结果。`kimi-code` 仓库实际约有 764 条 issue，因抓取层默认 `max_pages=10`，只取到最近 334 条有效 issue；`kimi-agent-sdk` 共 98 条，已全量抓取。该样本足以支撑趋势和 blocker 模式洞察，全量分析可调整分页参数。
2. **全量 432 条耗时**：body=1500 时约 18-20 分钟；为在合理时间内跑完真实分析，本次实际运行采用 body=800（仍满足"截断长日志"的设计目标）。
4. **证据链长度**：当前 evidence 是模型自由截取的原文片段，未限制最大长度，报告里做了 120 字截断展示。
5. **模型稳定性**：highspeed 偶尔返回空 content，当前靠重试 3 次兜底；若仍失败则整批标为 `other`。
6. **扩展项未做**：时间趋势、首次响应时长、自动生成文档条目清单。
