...
### 为什么用 `kimi-k2.7-code-highspeed` 而不是 K3？

我先用 K3 做了实验，发现对 14 条 × 800 字符的 batch，K3 在 180 秒超时窗口内经常无法返回。分类任务对推理深度要求不高，但对**稳定性和速度**要求高，因此选择 `kimi-k2.7-code-highspeed`。

> **面试话术**："我先试了 K3，批量长文本下超时严重；这个任务对推理深度要求不高、对稳定性和速度要求高，所以我选了 highspeed 版。这是任务匹配，不是 K3 不行。"
